...
public class BankIdService {
    ...
    public BankIdResult verify(String signatureBase64) {
        try {
            ...
            // Extract usrNonVisibleData (e.g. hash of agreement or information of a technical nature)
            ...
            // Extract srvInfo/name (Relying Party info - the organization the individual signed with)
    ...
    private List<String> verifyCertificateChain(List<X509Certificate> certs) {
        ...
        for (int i = 0; i < certs.size(); i++) {
            ...
            // Verify signature (except for last cert which is self-signed or signed by external root)
            if (i < certs.size() - 1) {
                try {
